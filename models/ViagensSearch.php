<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Viagens;

/**
 * ViagensSearch represents the model behind the search form of `app\models\Viagens`.
 */
class ViagensSearch extends Viagens
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['Id'], 'integer'],
            [['Data_Partida', 'Data_Chegada', 'Ponto_Partida', 'Ponto_Chegada'], 'safe'],
            [['Preco'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Viagens::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'Id' => $this->Id,
            'Data_Partida' => $this->Data_Partida,
            'Data_Chegada' => $this->Data_Chegada,
            'Preco' => $this->Preco,
        ]);

        $query->andFilterWhere(['like', 'Ponto_Partida', $this->Ponto_Partida])
            ->andFilterWhere(['like', 'Ponto_Chegada', $this->Ponto_Chegada]);

        return $dataProvider;
    }
}
